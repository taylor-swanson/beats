...
func (f *fetcherGraph) Groups(ctx context.Context, deltaLink string) ([]*Group, string, error) {
...
func (f *fetcherGraph) Users(ctx context.Context, deltaLink string) ([]*User, string, error) {
	var users []*User
...
func newFetcherGraph(cfg *config.C, logger *logp.Logger, auth Authenticator) (Fetcher, error) {
	var c graphConf
...
func newGroupFromAPI(g groupAPI) *Group {
	...
	for _, v := range g.MembersDelta {
		if v.Type == apiUserType {
			newGroup.Members = append(newGroup.Members, Member{
				ID:      v.ID,
				Type:    MemberUser,
				Deleted: v.Deleted(),
			})
